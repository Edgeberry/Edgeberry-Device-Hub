...
Set SERVICE to one of: mosquitto, core-service, api, provisioning-service, twin-service, application-service
Example:
...
case "$SERVICE" in
  ...
  twin-service)
    ...
    ;;
  application-service)
    start_node_service "${APP_DIR}/application-service" "application-service"
    ;;
  "")
    usage
    exit 2
